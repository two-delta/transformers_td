...
# Remove stale transformers_td.egg-info directory to avoid https://github.com/pypa/pip/issues/5466
stale_egg_info = Path(__file__).parent / "transformers_td.egg-info"
if stale_egg_info.exists():
    print(
        f"Warning: {stale_egg_info} exists.\n\n"
        "If you recently updated transformers_td to 3.0 or later, this is expected,\n"
        "but it may prevent transformers_td from installing in editable mode.\n\n"
...
# 2. once modified, run: `make deps_table_update` to update src/transformers_td/dependency_versions_table.py
_deps = [
    "Pillow>=10.0.1,<=15.0",
    "accelerate>=0.26.0",
    "av",
...
#
# python -c 'import sys; from transformers_td.dependency_versions_table import deps; \
# print(" ".join([ deps[x] for x in sys.argv[1:]]))' tokenizers datasets
#
# Just pass the desired package names to that script as it's shown with 2 packages above.
#
# If transformers_td is not yet installed and the work is done from the cloned repo remember to add `PYTHONPATH=src` to the script above
#
# You can then feed this for example to `pip`:
#
# pip install -U $(python -c 'import sys; from transformers_td.dependency_versions_table import deps; \
# print(" ".join([deps[x] for x in sys.argv[1:]]))' tokenizers datasets)
#
...
class DepsTableUpdateCommand(Command):
    ...
    def run(self):
        ...
        target = "src/transformers_td/dependency_versions_table.py"
...
setup(
    ...
    entry_points={
        "console_scripts": [
            "transformers_td=transformers_td.commands.transformers_cli:main",
            "transformers_td-cli=transformers_td.commands.transformers_cli:main_cli",
        ]
    },
